...
public class Deck {
    ...
    public Deck() throws IOException {
        ...
        this.dealDominoes();
        System.out.println("Игра началась. Вот ваши костяшки домино: ");
    ...
    public Player findPlayerWithDoubleSix(){
        for(Player player: this.players){
            for(int i = 0; i < 7; i++){
                if(player.getHand().get(i).toString().equals("1-1")){
                    return player;
                }
            }
        }
        return new Player("error");//1-1 not found
    }
    ...
    public Player nextPlayer(Player playerWithTurn){
        ...
        return new Player("ошибка");//If no next player found
    }

    public void printDominoesToPlay(Player player){
        System.out.println("Доступны доминошки: ");
        int i = 1;
    ...
    public void playDomino(Player player, Domino domino){
        ...
        System.out.println( player.getName() + " игрок: "  + domino + "\nСтол теперь:\n");
    ...
    public void next() throws IOException {
        ...
        if(plays > 2) {
            if (this.isGameBlocked()) {
                ...
                if (numberOfPlayersWithMinPoints > 1) {
                    ...
                    System.out.println(this.players[winner].getName().toUpperCase() + " ПОБЕДИЛ!! игра окончена");
                }
                return;
            }
        }
        ...
        if(!isDoubleSixOnTable && this.playerWithTurn.getName().equals(this.players[0].getName())){
            System.out.println("Теперь твоя очередь.");
            this.printDominoesToPlay(this.playerWithTurn);
            System.out.print("Какое домино вы хотели бы сыграть: ");
            int dominoPicked = scanner.nextInt();
            dominoPicked--;
            while(dominoPicked <= -1 || dominoPicked > this.players[0].getHand().size()-1){
                System.out.print("Неверный номер, пожалуйста, повторите попытку\nВ какое домино вы хотели бы сыграть: ");
                dominoPicked = scanner.nextInt();
                dominoPicked--;
            }
            while(this.players[0].findIndexOfDoubleSix() != dominoPicked){
                System.out.print("Предупреждение! Вы должны начать игру со счетом 1:1\nВ какое домино вы хотели бы сыграть\n: ");
                dominoPicked = scanner.nextInt();
                dominoPicked--;
            }
            ...
            System.out.println("Ты играешь: " + dominoPlayed);
            System.out.print("стол сейчас:\n");
            ...
            System.out.println("это " + this.playerWithTurn.getName() + "очередь ходить.");
            ...
            System.out.println(this.playerWithTurn.getName() + "сыграно" + dominoPlayed);
            System.out.print("стол на данный момент:\n");
            for(Domino domino: dominoesOnTable){
                System.out.print(domino + " ");
            }
            isDoubleSixOnTable = true;
            ...
            if(dominoesAvailableToPlay.size() == 0){
                while(dominoPicked != -2){
                    System.out.print("Ты не можешь играть этим домино.\nКаким ты хочешь сыграть: ");
                    dominoPicked = scanner.nextInt();
                    dominoPicked--;
                }
                System.out.println("Ты пропустил свою очередь");
                System.out.print("стол сейчас:\n");
                for (Domino domino : dominoesOnTable) {
                    System.out.print(domino + " ");
                }
                ...
                while(dominoPicked <= -1 || dominoPicked > this.players[0].getHand().size()-1 || !dominoesAvailableToPlay.contains(this.players[0].getHand().get(dominoPicked))){
                    if(dominoPicked == -2){
                        System.out.print("Вы не можете пропустить.\nКакое домино ты хочешь сыграть: ");
                    }
                    else{
                        System.out.print("Ты не можешь играть этим домино.\nКакое домино ты хочешь сыграть: ");
                    }
                ...
                if(plays > 1 && this.canPlayDominoOnBothSides(dominoPlayed, endRight, endLeft)){
                    System.out.print("Вы можете разыграть это домино с обоих сторон. Где вы ее разыграете? 1 слева, 2 справа: ");
                    int input = scanner.nextInt();
                    while(input != 1 && input != 2){
                        System.out.print("Вы можете разыграть это домино с обоих концов. С какой стороны разыграете? 1 слева, 2  справа: ");
            ...
            System.out.println("Это " + this.playerWithTurn.getName() + "Его очередь.");
            ArrayList<Domino> availableDominoes = this.getDominoesAvailableToPlay(this.playerWithTurn);
            if (availableDominoes.size() == 0) {
                System.out.println(this.playerWithTurn.getName() + " пропущенный поворот.");
                System.out.print("Стол на данный момент:\n");
                for (Domino domino : dominoesOnTable) {
                    System.out.print(domino + " ");
                }
        ...
        if(this.playerWithTurn.getHand().size() == 0){
            ...
            System.out.println(this.playerWithTurn.getName().toUpperCase() + " ПОБЕДИЛ!! игра окончена");
        }
